...
今天分享的是 Codex，重點不是想要說他有多猛

而是想跟大家說，培養多個工具切換的能力，不要被單一工具綁死

因為當一個工具紅起來後，在不久的將來往往會迎來失敗，歷史總是驚人的相似，幾乎沒有例外

此時如果你熟悉多個 AI 工具，也知道如何切換，那會是你的優勢
...
## 用 Vibe Coding 建立可重複使用的框架

框架的核心概念，就是不要讓 AI 每次都從零開始生成結果，而是讓它幫你建立一套「模板」，之後你只要放進不同的內容，就能產出穩定、一致的成果。

這邊我就拿自己當案例，受邀到企業內訓時，我每次都需要準備課程講義。假使都用 Vibe Coding 生成講義，那風格和品質都不可控；但如果有一套框架，那只要替換內容，就能快速產出符合我需求的講義。
...
references 資料夾放著 Markdown 格式的參考文件，而 scripts 則是存放框架編譯的程式腳本
...
如果你有跟著教學走到這一步，恭喜你建立了屬於自己的框架

但...這只是開始而已

## 自由度高的背後，是功能的不完善

儘管剛剛設計的 Agent Skill 搭配框架腳本可以輕鬆生成講義網頁。
...
這邊我只是舉出幾個常見痛點，雖然建立框架聽起來很厲害，但自由的背後，代表你是目前唯一的維護者

因為框架的細節是在實際使用後，你才知道哪裡要調整；儘管設計與程式撰寫可以交給 AI，但方向與美感還是要自己掌握。

這就是自己設計框架的代價。你想要主控權，就得承擔客製化背後的複雜度。

老實講，如果只是要有一個線上簡報工具，市面上已經有很多現有工具，不一定要走自建框架這條路。

而我選擇做這件事，與其說是建立一個更好的方案，不如說是在設計一套最符合自己需求的工具。

## 成果展示

這邊我就直接跟大家展示目前的成果，我已經使用這套框架講了 5 場 Vibe Coding 課程了
...
這邊我就選擇 exmaple 資料夾底下的範例 Markdown 文件來給大家示範

這個文件是這隻影片的 Markdown 逐字稿，我直接跟 AI 說
...
但由於這個框架是我建立的，所以我也能設計特殊的語法，比如「XXX」
...
雖然框架前期的建立有很多細節要處理，但建立好後，你就能根據自己的意志來優化細節，不再被平台綁架
...
自動生成 commit，並透過 Push Branch 將內容更新到 GitHub 上面

接著來到 GitHub 頁面，如果你想要讓網頁被大家瀏覽，只要去 Settings 頁面

這次我們用預設的 Deploy from a branch，並把 Branch 切換到 master 就可以了

因為前面生成的是單純的 html 檔案，可以直接顯示

此時到 Actions 的頁面就會看到有成功觸發，順利的話，就會看到部署完成後的網址

但點開來後，你會發現頁面不存在，這是因為網頁檔是存在於 example 資料夾底下

我們回到 Code 頁面，我們的檔案在 example 底下

所以只要在網址後面加上 example 就可以順利顯示嚕～

未來，你就可以在不同的資料夾下建立對應的線上課程網頁
...
另外有些資訊如果希望反覆利用，不需每次都重新貼上；我們就可以把它抽出來，放到 config 資料夾，讓他可以在所有頁面被共用。
...
其實我是用系統設計的角度，來思考這個框架如何設計會更好用

像變動性高的資訊，我把它放在 Markdown 文件方便修改，共用的資訊則設計成 config 來進行維護。
...
第二，把 AI 的隨機輸出，轉成可用 Markdown 維護的框架

第三，把框架打包成 Agent Skill，讓它可以重複使用。

在走完這整個流程之後，我感觸最深的，是發現 AI 真的可以把自己腦中的工作流，一點一點變成一套系統。

而這個任務放在過去，是需要一個團隊，花好幾週的時間才有可能做到的。

但這次我只是在備課的過程中突然靈光乍現，然後就透過 Vibe Coding 完成了框架的雛形。
...
也在這個時候，Vibe Coding 的成果正式從玩具走向產品。

我認為 AI 真正厲害的地方，從來都不是幫你省下一次工作的時間。

而是它開始讓你有機會，把自己原有的工作流，設計成一套可以持續優化的框架，這才是 Agent Skill 最大的魅力所在。
...
如果只看 Token 額度的大方程度，目前 Codex 是最多的，但這是暫時